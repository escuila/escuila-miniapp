...
(function () {
  ...
  function saveMeCache(me) {
    // احفظ بيانات المستخدم في CloudStorage مع timestamp (TTL = 24 ساعة)
    try {
      var payload = JSON.stringify({ ts: Date.now(), me: me });
      if (tg && tg.CloudStorage) {
        tg.CloudStorage.setItem(ME_CACHE_KEY, payload, function () {});
      } else {
        localStorage.setItem(ME_CACHE_KEY, payload);
      }
    } catch (e) { /* silent */ }
  }

  function loadMeCache(cb) {
    // استرجع بيانات محفوظة (صالحة خلال 24 ساعة)
  ...
  function renderLibrary() {
    ...
    if (state.online && state.me && state.me.vip) {
      ...
      vipRow.type = 'button';
      var vli = el('div', 'vip-library-icon');
      vli.appendChild(svgIcon('gem', 18));
  ...
  function renderFileDetails(f) {
    ...
    if (access === 'free' && /^https:\/\/t\.me\//i.test(f.u)) {
      ...
      if (isVipUser) {
        // ─── مشترك VIP: زر واحد مباشر بدون تردد ───
        ...
        var openReq = el('button', 'secondary-btn');
        openReq.type = 'button';
        openReq.appendChild(svgIcon('send', 17));
        openReq.appendChild(el('span', null, 'طلب الملف من البوت'));
        openReq.addEventListener('click', function () { openInBot(f); });
        box.appendChild(openReq);
